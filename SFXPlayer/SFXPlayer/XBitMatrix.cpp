...
XBitMatrix::XBitMatrix(uint32_t nWidth, uint32_t nHeight, uint8_t nKeySize)
{
	m_nMtxWidth = nWidth;
...
void XBitMatrix::SetValue(uint32_t x, uint32_t y, uint32_t value)
{
...
uint32_t XBitMatrix::GetValue(uint32_t x, uint32_t y)
{
	uint32_t nCurIndex{ 0 };
	uint8_t  nCurOffset{ 0 };
...
void XBitMatrix::MapToIndex(uint32_t x, uint32_t y, uint32_t& index, uint8_t& offset)
{
